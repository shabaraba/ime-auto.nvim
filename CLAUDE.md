...
│   ├── ime.lua               # IME制御コアロジック（キャッシング・デバウンス・状態管理）
...
- **デバウンス**: `mode_change_timer` で100msデバウンス

**プラットフォーム別実装**（`ime-platform.lua`）:
- macOS: `M.macos()` から `swift-ime-tool.lua` 経由で Swift ツール呼び出し
- Windows: `M.windows()` から PowerShell スクリプト実行
- Linux: `M.linux()` から `fcitx-remote` または `ibus` コマンド実行
